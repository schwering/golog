...
prop_max_sq seed = let result = run_pso seed 50 5 (-5, 5) (Max (\x -> -x*x))
                   in -0.1 < result && result < 0.1

prop_max_sq2 :: Bool
prop_max_sq2 = let result = run_pso 10 100 5 (-5, 5) (Max (\x -> -x*x))
               in -0.01 < result && result < 0.01

prop_min_sq :: Int -> Bool
prop_min_sq seed = let result = run_pso seed 50 5 (-5, 5) (Min (\x -> x*x))
                   in -0.1 < result && result < 0.1

prop_min_abs :: Int -> Bool
prop_min_abs seed = let result = run_pso seed 50 5 (-2, 2) (Min abs)
                    in -0.1 < result && result < 0.1

prop_min_abs2 :: Bool
prop_min_abs2 = let result = run_pso 10 50 5 (-100, 100) (Min abs)
                in -0.1 < result && result < 0.1

prop_max_poly :: Bool
prop_max_poly = let result = run_pso 10 1000 5 (-1000, 1000) (Min (\x -> x*x*x*x - 5.0 * x*x + 4.0))

...
instance BAT Int where
   data Sit Int = Sit [Int] deriving Show
   s0 = Sit []
   do_ a ~(Sit as) = Sit (as ++ [a])
   poss n (Sit as) = n == length as
...
testLazy = take 10 as
   where Sit as = fake4
         ...
         --fake3' s p' = fake3' (do_ (case p' of ~(Seq ~(PseudoAtom ~(Atom ~(Prim a))) _) -> a) s) (case p' of ~(Seq _ p) -> p)
         fake3' s p' = fake3' (do_ (2::Int) s) undefined

...
type OptiF u v = u -> (u -> v) -> u

data Tree v a = Empty
              | Leaf a
              | Parent a (Tree v a)
              | Branch (Tree v a) (Tree v a)
              | forall u. Sprout (OptiF u v) u (u -> Tree v a)
...
instance Functor (Tree v) where
   ...
   fmap f (Sprout opti x0 t) = Sprout opti x0 (\x -> fmap f (t x))
...
force val inf (Sprout opti x0 t) = force val inf (t (opti x0 val'))
...
lmap f (Sprout opti x0 t) = Sprout opti x0 (\x -> lmap f (t x))
